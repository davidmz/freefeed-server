...
export function initHandlers(jobManager) {
  ...
  // "Saga" handlers. This handlers should not contain business logic but only
  // create next jobs.
  jobManager.onComplete(USER_COOLDOWN_START, async (_, user) => {
    if (!user) {
      return;
    }
  ...
  jobManager.onComplete(
    USER_DELETION_START,
    (_, user) =>
      user.goneStatus === GONE_DELETION &&
      // Start actual data deletion
      Job.create(USER_DELETE_DATA, { id: user.id, email: user.hiddenEmail })
  );

  jobManager.onComplete(
    USER_DELETE_DATA,
    // Repeat this job if we are not done
    (job, isDone) => isDone || Job.create(job.name, job.payload)
  );
}
...
const checkUserStatus = (desiredStatus, handler) => async (job) => {
  ...
  if (user.goneStatus === desiredStatus && user.goneAt?.getTime() === goneAt) {
    ...
    // Return user if handler was executed
    return user;
  }

  // User status changed after the job creation so skip this job
  return null;
};
